...
<body>
    ...
    <script>
        ...
        const COMPANY = 'Tiny COM'; URL = { WHATSAPP:'https://api.whatsapp.com/send?phone=|phone|&text=|text|',SOURCE_LINK: '{{ route('source_link','|uuid|') }}',BILL_LINK: '{{ route('bill_link','|uuid|') }}' };
        function urlParse(item,data){ return URL[item].replace(/\|(\w+)\|/g,(f,m) => data[m] || '') }
