...
const getters = {
    all({ SOURCES },{ source_orders },rootState){ return _.map(SOURCES,source => {
        return {
            ...
            media: _.map(source.media,media => _.pick(media,['id','file_name'])),
        }
    }) },
...
const actions = {
    ...
    create({ state,commit },data){
        return new Promise(resolve => {
            $.post(state.url.create,data).then(function({ data }){
                commit('add',data); resolve(data);
            })
        });
    },
    extend({ state,commit },data){
        return new Promise(resolve => {
            $.post(state.url.extend,data).then(({ data }) => { commit('update',data); return resolve(data); })
        });
    },
    sync({ state,dispatch,commit }){
        $.post(state.url.list).then(({ data }) => {
            if(data && data.length){ _.forEach(data,source => commit('update',source)) }
        }).catch(() => null).then(() => setTimeout((dispatch) => dispatch('sync'),state.interval.list,dispatch))
    }
}
