...
const mutations = {
  ...
  extra(state,data){ _.forEach(data,(val,key) => state[key] = val) }
}
const actions = {
  ...
  sync({ state:{ url,items,uuid,sync,timeout },commit,dispatch }){
    $.post(url.sync, { items }).then(({ data }) => {
      if(uuid !== data.uuid) commit('init',data.uuid);
      if(data.items && data.items.length !== items.length || _.sumBy(items,({quantity}) => _.toNumber(quantity)) !== _.sumBy(data.items,({quantity}) => _.toNumber(quantity))){
        _.forEach(items,({ item }) => commit('removeItem',_.toInteger(item)));
        _.forEach(data.items,({ item,quantity }) => { let qty = _.toNumber(quantity); while(qty-- >= 1) dispatch('increment',item); });
      }
