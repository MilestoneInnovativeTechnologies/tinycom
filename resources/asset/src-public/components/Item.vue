<template>
  <q-card>
    <q-card-section horizontal>
      ...
      <q-card-section class="q-pa-xs q-pb-md text-primary fit row wrap flex-center" style="min-height: 7rem">
        <div class="col-12 q-pt-sm" style="height: 3rem; line-height: 1.25" :style="prop('name').length > 35 ? { fontSize:'0.8rem' } : {}" @click="$router.push({ name:'item',params:{ id } })">
          <div class="float-right" v-if="sale || exclusive" style="margin-top: -0.5rem">
        ...
        <div class="col-grow" @click.prevent="$router.push({ name:'item',params:{ id } })">
          <span style="font-size: 1.1rem">{{ price | number }}</span><q-spinner-rings v-if="exclusive" color="primary" />
          <del v-if="sale || exclusive" class="block text-grey" style="line-height: 0.4">{{ prop('price') | number }}</del>
        </div>
