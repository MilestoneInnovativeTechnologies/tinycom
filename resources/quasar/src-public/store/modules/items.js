...
const actions = {
  ...
  fetch({ state,commit },id){ $.post(state.url.fetch,{ id }).then((item) => (item && item.id) ? commit('replace',item) : null) },
  sync({ state,dispatch,commit }){
    $.post(state.url.sync)
      .then(({ data }) => { if(data && data.length){ _.forEach(data,item => commit('update',item)) } })
      .catch(() => null)
      .then(() => setTimeout((dispatch) => dispatch('sync'),state.timeout,dispatch))
  },
  create({ state,commit },data){
    return new Promise(resolve => $.post(state.url.create, data).then(({ data }) => { commit('add',data); resolve(data) }))
  },
  stock({ state,commit },stock){
    let data = new FormData(); data.append('stock',stock);
    return new Promise(resolve => $.post(state.url.stock,data).then(({ data }) => commit('stock',_.toInteger(data),{ temp:resolve(data) })))
  },
