...
export default {
  ...
  props: ['filter'],
  computed: {
    items(){ return _(this.$store.getters["ITEMS/all"]).filter(({ status }) => status === 'N').filter(({ name,description }) => _.isEmpty(this.filter) || _.includes(_.toLower(name+description),_.toLower(this.filter))).value() },
  }
}
</script>
