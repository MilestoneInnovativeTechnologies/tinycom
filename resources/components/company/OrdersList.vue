    ...
    <b-container>
        <b-table :items="orders" :fields="fields" small show-empty :empty-text="empty_text">
            ...
            <template v-slot:cell(actions)="data">
                ...
                <b-button size="sm" v-if="isAct('pay',data.item)" variant="primary" @click="pay(data.item.id,data.item.company)"><b-icon icon="forward-fill"></b-icon> Pay Now</b-button>
                <b-button size="sm" v-if="isAct('cancel',data.item)" variant="danger" :disabled="cancel_disable" @click="cancel(data.item.id)"><b-icon icon="x"></b-icon> Cancel</b-button>
            </template>
        </b-table>
        <b-modal title="New payment" id="orders-pay-order-modal" hide-footer size="sm"><payment-form :order="pay_order" @submitted="paid"></payment-form></b-modal>
    </b-container>
</template>
...
<script>
    export default {
        ...
        methods: {
            ...
            pay(order,company){ this.pay_order = order; this.pay_order_company = company; this.$bvModal.show('orders-pay-order-modal') },
            paid({ message,variant }){ this.$bvModal.hide('orders-pay-order-modal'); this.$bvToast.toast(message,{ variant,title:'Payment' }); this.$post('subscription','fetch',{ company:this.pay_order_company }); },
