...
<script>
    export default {
        ...
        methods: {
            submit(){ this.disabled = true; this.$post('referrer',this.action,this.post).then(this.submitted); },
            submitted(data){ this.disabled = false; this.$emit('response',data) }
        },
        created() {
            if(this.$attrs['referrer']){
                let referrer = this.$attrs['referrer'];
                this.action = 'update'; _.forEach(this.fields,key => this[key] = referrer[key] )
            }
        }
    }
</script>